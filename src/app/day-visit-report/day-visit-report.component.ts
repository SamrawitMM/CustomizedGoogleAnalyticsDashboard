...
@Component({
  selector: 'app-day-visit-report',
  templateUrl: './day-visit-report.component.html',
  styleUrls: ['./day-visit-report.component.css']
})
export class DayVisitReportComponent implements OnInit {
...
setTimeout(
  ...
  //   //  console.log(date.substring(4, 6) -1, " month")
  //   //  console.log(date.substring(6,9), "day")
  //   return format(
  //     new Date(
  //       date.substring(0, 4),
  //       date.substring(4, 6) - 1,
  ...
  transformToDate (date: string | any)  {
    return new Date(
      date!.substring(0, 4),
      date!.substring(4, 6) - 1,
      date!.substring(6, 8)
...
//       // this.data = this.queryReport.getAnalytics(myReq);
  ...
  drawChart(start_date:any, end_date:any) {
    ...
    console.log("                I AM HERE        THIS FUNCTION IS CALLED")
    if(this.lineChart && this.reportData.labels && this.reportData.values ){
      this.lineChart.destroy();
      this.reportData.labels = []
      this.reportData.values = []
...
// });
  ...
  this.qs.getAnalyticsData(myReq).subscribe((report) =>{ 
    if(report && Object.keys(report).length > 0){
      ...
      console.log(report.body.reports[0].data.rows)
      let qr= report.body.reports[0].data.rows;
      this.queryResult  = qr
      console.log(this.queryResult)
      if(!this.isUndefined() && this.queryResult.length > 0 && this.queryResult){
      console.log(this.queryResult)
